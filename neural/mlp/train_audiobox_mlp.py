...
def main():
    ...
    # Ovo sprečava da gigantski izvođači sa 40+ miliona pratilaca uguše ostale podatke
    X[:, 5] = np.log1p(X[:, 5])
    ...
    print("\n--- Rezultati evaluacije MLP modela (MinMax + Log Followers) ---")
    print(f"RMSE (Root Mean Squared Error): {rmse:.4f}")
    print(f"R² Score (Koeficijent determinacije): {r2:.4f}")

    # Čuvanje modela i skalera za inference
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), MODEL_DIR / "hit_predictor_mlp.pth")
    with open(MODEL_DIR / "feature_scaler.pkl", "wb") as f:
        pickle.dump(scaler, f)
    print(f"\nModel i skaler su uspešno sačuvani u: {MODEL_DIR}")
